...
from timetank_annotation.multi_pass_annotator import TemporalAnnotatorReAct
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Annotate a temporal corpus with a multi-agent annotation and verification loop."
    )
    parser.add_argument("--input", default="data/TimeTankSample.csv", help="Path to the input CSV corpus.")
    parser.add_argument(
        "--output-dir",
        default="generation_results/multi_pass",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Maximum number of annotation-review rounds per sentence.",
    )
    return parser
...
async def annotate_one_model(
    *,
    ...
    artifact_slug = f"{model_slug}_ReAct"
    annotation_path = config.output_dir / "annotations" / f"{artifact_slug}.csv"
    log_path = config.output_dir / "logs" / f"{artifact_slug}.jsonl"
    summary_path = config.output_dir / "summaries" / f"{artifact_slug}.summary.json"
    messages_dir = config.output_dir / "messages" / artifact_slug

    annotator = TemporalAnnotatorReAct(
    ...
    summary = {
        "annotation_mode": "multi_agent_review_loop",
        "model_name": model_name,
        "reviewer_model_name": annotator.reviewer_model_name,
        ...
        "accepted_by_reviewer_rows": 0,
        "started_at_unix": time.time(),
        "last_processed_row_index": 0,
    }

    print(
        f"[MODEL START] {model_name} mode=multi-agent rows={len(rows)} temperature=1.0 "
        f"reasoning={config.reasoning_effort or 'off'} review_rounds={max_review_rounds}",
        flush=True,
    )
    ...
    with AnnotationCsvWriter(annotation_path, fieldnames) as csv_writer, JsonlLogger(log_path) as logger:
        for row_index, row in enumerate(rows, start=1):
            ...
            try:
                ...
                summary["review_rounds_total"] += outcome.review_round_count
                if outcome.accepted_by_reviewer:
                    summary["accepted_by_reviewer_rows"] += 1
                ...
                print(
                    f"[{artifact_slug}] {row_index}/{len(rows)} success "
                    f"attempts={outcome.attempt_count} rounds={outcome.review_round_count} "
                    f"accepted={int(outcome.accepted_by_reviewer)} "
                    f"annotations={len(outcome.annotations)} row_tokens={usage_stats['total_tokens']} "
                ...
                print(
                    f"[{artifact_slug}] {row_index}/{len(rows)} error "
    ...
    _write_summary(summary_path, summary)
    print(
        f"[MODEL DONE] {model_name} mode=multi-agent ok={summary['successful_rows']} "
        f"err={summary['failed_rows']} out_rows={summary['total_output_rows']} "
        f"rounds={summary['review_rounds_total']} accepted={summary['accepted_by_reviewer_rows']} "
        f"tokens={summary['usage_total_tokens']}",
        flush=True,
    )
...
async def main() -> None:
    ...
    print(f"All multi-agent outputs written to: {config.output_dir}")
...
def _write_summary(path, summary: dict[str, object]) -> None:
